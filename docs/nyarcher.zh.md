...
## 免责声明
该脚本旨在尽可能在任何 Linux 发行版上提供与 Nyarch Linux 相似的体验的同时不修改系统文件。因此，请阅读[你错过了什么部分](#what-you-are-missing)哦喵～。
此外，某些应用程序，特别是 Nyarch 脚本，可能在非基于 Arch 的发行版上面有可能无法正常工作的喵～。
...
```
注意：wal 需要位于 /usr/bin/wal，这是最后一条命令的作用
...
- 失去在物理机上安装 Nyarch 并且希望它不会摧毁你的猫猫电脑的勇气
...
- 你会遇到一些在 Nyarch 上不会出现的预期错误：
  ...
  - Material You 可能无法在某些发行版上自动应用 Material You Gnome Shell 主题（在 Fedora 中遇到），而是需要登录 / 注销 才可以生效。目前尚未找到简单的修复方法。一个无需注销的解决方法是，在 gnome tweaks（或扩展设置）中将 Gnome shell 主题更改为另一个，然后再切换回 Material You 主题
