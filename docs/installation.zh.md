...
- 至少 4GB 容量的 U 盘
- 一台已能正常工作的电脑（或者如果你使用 [Etchdroid](https://etchdroid.app/ ) 的话，安卓设备也可以的喵～）
...
按照Ventoy官方制作好Ventoy启动盘，将Nyarch的镜像放置到你的移动猫盘，随后在启动的Ventoy中选择ISO镜像喵～
...
2. 访问设备的 BIOS/UEFI 工具。访问方式取决于你的设备。
  如果你不知道如何访问 BIOS，在设备启动时狂按 F 键并大喊 *欧拉欧拉欧拉欧拉欧拉！！！* ，直到按对为止。
3. 禁用安全启动
4. 找到启动顺序设置，将你的 U 盘放在第一位。或者，如果你有访问启动菜单的权限，从那里启动。
5. 保存并退出
6. 希望 Nyarch Live 能启动。如果不能，检查你是否确实将 U 盘设为第一位，然后在 Nyarch Support 频道（或任何 Linux 社区支持）中化身猫娘求助喵～。

### Nyarch Live Environment
    Nyarch Live 环境
你* 成功地 * 已成功启动 Nyarch 了喵～。 
现在你处于 **Live 环境** 中。 
...
如果你在 Live 环境中需要执行需要输入用户密码的操作，密码是 `live` 哦喵～。
#### Nyarch 安装程序
安装程序将在系统启动时自动启动。

![immagine](https://github.com/user-attachments/assets/d294db49-5fd7-4844-9083-59790998dcc7 )

然后它会提示你输入本地化设置。只需要根据猫猫最喜欢的偏好填写就可以了喵～。

##### 分区
**注意**：这部分要非常小心，因为错误可能导致数据丢失！
...
Nyarch 大量使用动画来让你有更顺畅的猫猫电脑，所以**在虚拟机中启用 3D 加速**。否则它会卡得要死掉了喵～。猫猫你不会想看比 Blue Lock S2 更差的动画的，相信我。
使用以下规格创建虚拟机：
...
创建并启动虚拟机后，跳到[Nyarch Live 环境](#nyarch-live-environment) 喵～